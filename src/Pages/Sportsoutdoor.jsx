...
const SportsandOutdoor = () => {
    ...
    const SportsandOutdoor = useSelector(
        (state) => state.Sliderproduct.categories?.SportsandOutdoor?.products || []
    );
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setCurrent((prev) => (prev + 1) % SportsandOutdoor.length);
        }, 5000);
        return () => clearInterval(interval);
    }, [SportsandOutdoor.length]);

    return (
        <div className="w-full relative overflow-hidden bg-black">
            <div className="w-full relative h-[380px]">
                {SportsandOutdoor.map((product, index) => (
                    <div
                        key={index}
